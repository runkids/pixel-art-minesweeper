...
const MONSTER_TEMPLATES: Omit<Monster, 'id'>[] = [
  ...
  { name: 'Bat', emoji: '🦇', hp: 8, maxHp: 8, attack: 3, defense: 0, exp: 6, gold: 4, isBoss: false, description: 'A bat flying in the darkness' },
  ...
  { name: 'Frost Giant', emoji: '🧊', hp: 35, maxHp: 35, attack: 6, defense: 4, exp: 30, gold: 25, isBoss: false, description: 'A freezing giant monster' },
...
const BOSS_TEMPLATES: Omit<Monster, 'id'>[] = [
  { name: 'Dungeon Lord', emoji: '👹', hp: 50, maxHp: 50, attack: 10, defense: 3, exp: 100, gold: 50, isBoss: true, description: 'The powerful lord of this dungeon' },
  ...
  { name: 'Lich King', emoji: '🧙', hp: 70, maxHp: 70, attack: 14, defense: 3, exp: 180, gold: 90, isBoss: true, description: 'A powerful undead sorcerer' },
  { name: 'Cerberus', emoji: '🐕‍🦺', hp: 90, maxHp: 90, attack: 11, defense: 6, exp: 250, gold: 120, isBoss: true, description: 'The three-headed guard of hell' },
]
...
const TREASURE_TEMPLATES: Omit<Treasure, 'id'>[] = [
  ...
  { name: 'Greater Health Potion', emoji: '⚗️', type: 'potion', value: 6, description: 'Restores full HP' },
  { name: 'Skill Scroll', emoji: '📜', type: 'skill', value: 1, description: 'Learn a new skill' },
  { name: 'Mysterious Key', emoji: '🗝️', type: 'key', value: 1, description: 'A key to open chests' },
]
...
const SKILL_TEMPLATES: Skill[] = [
  ...
  { id: 'shield', name: 'Shield', emoji: '🛡️', description: 'Block the next mine damage', cooldown: 5, currentCooldown: 0, manaCost: 2, effect: 'shield' },
  { id: 'bomb', name: 'Bomb', emoji: '💣', description: 'Safely reveal a 3x3 area', cooldown: 8, currentCooldown: 0, manaCost: 3, effect: 'bomb', charges: 1 },
  { id: 'heal', name: 'Heal', emoji: '💚', description: 'Restore 2 HP', cooldown: 4, currentCooldown: 0, manaCost: 2, effect: 'heal', charges: 2 },
  { id: 'reveal', name: 'Reveal', emoji: '👁️', description: 'Show all mine positions for 3s', cooldown: 10, currentCooldown: 0, manaCost: 4, effect: 'reveal' },
]
...
export const useDungeon = defineStore('dungeon', () => {
  ...
  const attackMonster = (): { damage: number; killed: boolean; rewards?: { exp: number; gold: number } } => {
    ...
    if (currentMonster.value.hp <= 0) {
      ...
      combatLog.value.push(`Defeated ${currentMonster.value.name}! Gained ${rewards.exp} EXP and ${rewards.gold} gold`)
  ...
  const monsterAttack = (): number => {
    ...
    if (hasShield.value) {
      ...
      combatLog.value.push(`🛡️ Shield blocked the attack!`)
      return 0
    }
  ...
  const levelUp = () => {
    ...
    combatLog.value.push(`🎉 Level up! Level ${level.value}`)
  }
